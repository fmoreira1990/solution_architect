...
| A12 | Feature flag por percentual de tráfego + rollback sem deploy | `CTX-11` | médio |
| A13 | Job de reconciliação: pedidos presos, outbox parado, reservas órfãs | 0007 | médio |
...
| Multi-região com residência de dados | `CTX-09`, **ADR-0006 pendente** |
| Multi-moeda | `CTX-01` |
...
- PII do segundo país atendendo residência
...
5. **Estoque e Pagamento são premissa.** O enunciado não os menciona. Se precisarem ser construídos, a onda 30 não cabe em 30 dias.

## Pendências registradas

- A decomposição da onda 30 (A1–A14 + P1–P3) alimenta `docs/delivery/estimativa-fase1.md`. O esforço sai daí e a composição do time sai do esforço — não o contrário.
- As datas do Gantt são ilustrativas, ancoradas em um início hipotético em 2026-10-01.
- `ADR-0006` (residência de dados) continua pendente e é pré-requisito da onda 90.
